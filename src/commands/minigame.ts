...
const REQUIRED_PLAYERS = 2; // (CHANGE IN PRODUCTION) required players in order for the game to start and continue
    ...
    if (subcommand === "wordbomb") {
      ...
      collector.on("collect", async (i: ChatInputCommandInteraction) => {
        ...
        if (players.length > 1) {
          ...
          joinEmbed.setFields({
            name: "Players",
            value: `${players.map((player) => player.Member).join("\n")}`,
          });
        } else {
          joinEmbed.addFields({
            name: "Players",
      ...
      setTimeout(async () => {
        if (players.length < REQUIRED_PLAYERS) {
          interaction.channel.send(
            `Atleast ${REQUIRED_PLAYERS} people are required for this game 😔`
          );
          return;
        }
        ...
        while (players.length >= REQUIRED_PLAYERS) {
          for (let i in players) {
            ...
            const filter = (msg: Message) =>
              msg.member.id === currentPlayer.Member.id;
            const collector = channel.createMessageCollector({
              filter,
              time: WORDBOMB_TURN_TIME,
            });

            collector.on("collect", async (msg: Message) => {
              let wordIsValid: Boolean = await validateWord(
                msg.content.toLowerCase()
              );
              if (wordIsValid && msg.content.includes(subString)) {
                if (msg.content.toLowerCase() in usedWords) {
                  channel.send(
                    "Can't use a word that has already been used, -1 Chance."
                  );
                  wordIsValid = false;
                } else {
                  ...
                  channel.send(
                    `${currentPlayer.Member} **Correct! 👍**, your current score is **${currentPlayer.Score}**`
                  );
                }
              } else {
            ...
            if (!guessedCorrectly) {
              ...
              if (currentPlayer.Chances <= 0) {
                const index = players.findIndex(
                  (p) => p.Member.id === currentPlayer.Member.id
                );
                ...
                channel.send(
                  `**No more chances left, you have been Eliminated ❌**`
                );
              } else {
        ...
        if (players.length > 0) {
          ...
          interaction.channel.send(
            `The Winner is: ${
              players[0].Member.user
            } 🥳🏆\n\n**Scoreboard:** ${scoreboard
              .map((scores) => `${scores.Member}: ${scores.Score}`)
              .join("\n")}`
          );
        } else {
      ...
      await interaction.editReply({content: "Invalid game"});
    }
  },
};
